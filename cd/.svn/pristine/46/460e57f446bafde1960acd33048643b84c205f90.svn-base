 ...
 * @author xuph
 */
public class ExeScriptService {
	@Inject private BuildScriptDaoService buildScriptDaoSrv;
	@Inject private ServerCommonService serverCommonService;
	@Inject private BuildTaskLogGenService buildTkLogSrv;
	@Inject private ModuleCommonService moduleCommonService;
	@Inject private UuParamDaoService uuParamDaoSrv;
	@Inject private InstanceExeDaoService instanExeDaoSrv;
	@Inject private DtSocService dtSocService;
	@Inject private UuSocDaoService uuSocDaoSrv;
	@Inject private PublishTaskInstanceService publishTaskSrv;
	@Inject private JDBCRCallService jdbcCallSrv;
...
//				ModuleSourceInfo msi = new ModuleSourceInfo(dtsoc.getProtocol_type(),dtsoc);
...
//	}
	 ...
	 * @return
	 */
//	public void saveBuildExeScriptMsg(SCRIPT_METHOD script_method,SCRIPT_TYPE script_type, List<UuParamInfo> param_list,List<UuSocInfo> soc_list, String[] script_text, String work_id,long scirpt_bk_seq, String exe_user_id, String comp_cn_name,String comp_id) {
//		// 数据准备
...
//				for (String cmd : script_text) {str.append(cmd);str.append("\n");}
//			script_data.setScript_text(str.toString());
//		}
//		if (script_method == SCRIPT_METHOD.SQL) {
//			script_data.setScript_method(SCRIPT_METHOD.SQL);
//			StringBuffer str = new StringBuffer();
//				for (String cmd : script_text) {str.append(cmd);}
//				List<String>sql_list = splitSql(str.toString());
//			    for (String strs : sql_list) {checkSql(strs);}
//			script_data.setScript_text(str.toString());
//		}
//		if (script_method == SCRIPT_METHOD.COMP) {
//			// 参数UUID
...
//			ModuleInfo module = XmlReader.readerModule(comp_id);
//			script_data.setImpl_type(module.getImpl_type());
//		}
//		// 保存实例执行信息表
...
//	}
	 ...
	 * @param script_type
	 */
	public void checkExeScriptIsExist(String work_id, long script_bk_seq,
			SCRIPT_TYPE script_type) {
		if (Assert.isEmpty(buildScriptDaoSrv.getScriptByKey(work_id,
...
//	}
						...
						for (String sql : sql_list) {
						   SQL_TYPE type = getSqlType(sql);
							if(type==SQL_TYPE.SELECT){
								...
								if(mark==false){state = false;break;}
							}else{
	...
	private boolean executeQuerySql(String sql, String soc_name, String inst_id, int i, long script_bk_seq, String work_id, SCRIPT_TYPE script_type) {
		...
		try {
			...
			// 更新执行信息
			instanExeDaoSrv.updateExeMsgByKey(msg, inst_id, i);cmsvc.getSession().commitAndResume();
	...
	private Statement checkSql(String sql) {
		try {
			...
			throw new SqlParserException().addScene("SQL", sql).addScene(
					"REASON", "语法错误");
		}
	}
	...
	private List<String> splitSql(String sql_txt) {
		...
		if (sql_txt.contains(";")) {
			...
			for (String sql : strs) {
				if (!Assert.isEmpty(sql) && sql.trim().length() != 0
						&& !sql.trim().equals("")) {
					sql_list.add(sql);
				}
			}
	 ...
	 * @return
	 */
	private void checkModuleType(ModuleInfo info, DtSourceInfo dtsoc) {
		...
		}else if (IMPL_TYPE.SVN == info.getImpl_type()) {
			if (PROTOCOL_TYPE.SVN != dtsoc.getProtocol_type()) {
				throw new ExeCompModuleTypeExistException().addScene("SOC",dtsoc.getSoc_name());
			}
		}else if(IMPL_TYPE.WAS == info.getImpl_type()){
			if (PROTOCOL_TYPE.WAS != dtsoc.getProtocol_type()) {
				throw new ExeCompModuleTypeExistException().addScene("SOC",dtsoc.getSoc_name());
			}
		}else if(IMPL_TYPE.FTP == info.getImpl_type()){
	 ...
	 * @return
	 */
//	public String getComponentInst(List<UuParamInfo> param_list,List<UuSocInfo> soc_list,String comp_id){
//		List<PhasePublishBean> phase_list = new ArrayList<PhasePublishBean> ();
//		TemplateInfo template_info = new TemplateInfo();
//		ModuleInfo comp = XmlReader.readerModule(comp_id);
//		template_info.addSub_moduleInfo(comp);
//
//		StringBuffer sb = new StringBuffer();
//		for(UuSocInfo soc : soc_list){
//			DtSourceInfo dtsource = dtSocService.getInfoByKey(soc.getExe_soc_name());
//			checkModuleType(comp, dtsource);
//			sb.append(soc.getExe_soc_name()+",");
//		}
//		SrvSocBean [] ssb = SrvSocBean.getArray(sb.toString(), "");
//		PhasePublishBean ppb1= new PhasePublishBean();
//		ppb1.setGen_flag(YN_FLAG.YES);
//		ppb1.setImpl_type(comp.getImpl_type());
//		ppb1.setSrv_soc(ssb);
//		phase_list.add(ppb1);
//		
...
//	}
	 ...
	 * @return
	 */
	/*private String getScriptLogDirectory(String work_id) {
		String file_path = "";
		...
		if (web_root_dir.endsWith("/")) {
			file_path = web_root_dir + "task/" + work_id + "/";
		} else {
			file_path = web_root_dir + "/task/" + work_id + "/";
		}
		File file = new File(file_path);
	 ...
	 * @param script_type
	 */
    /* public void exe(String work_id, long script_bk_seq, SCRIPT_TYPE script_type) {
		BuildScriptInfo bscript = buildScriptDaoSrv.getScriptByKey(work_id,script_bk_seq, script_type);
		SCRIPT_METHOD script_method = bscript.getScript_method();
		List<UuSocInfo> soc_list = new ArrayList<UuSocInfo>();
		String uuid = bscript.getSoc_uuid();
		soc_list = uuSocDaoSrv.queryListInfoByUU(uuid);
		String[] script_text = bscript.getScript_text().split("\n");
	//	String comp_id = bscript.getComp_id();
	//	String param_uuid = bscript.getComp_param_uuid();
	//	List<UuParamInfo> param_lists = uuParamDaoSrv.getInfoByUuid(param_uuid);
      //  ParamInfo[] param_list = UuParamInfo.copyToParamInfos(param_lists);
		String inst_id = bscript.getInstance_id();
		TemplateInfo template = new TemplateInfo();
		DefaultEnv env = new DefaultEnv();
		int i = 0;
		if (script_method == SCRIPT_METHOD.SHELL) {
			for (UuSocInfo exsoc : soc_list) {
				...
				mdl.setCmds(script_text);
				mdl.setImpl_type(IMPL_TYPE.SHELL);
				mdl.setDt_source(dtsoc);
				template.addSub_moduleInfo(mdl);
				env.addStageNode(i, dtsoc);
				i++;
			}
		}
		TemplateInfo template = new TemplateInfo();
		...
		int i = 0;
		if (script_method == SCRIPT_METHOD.COMP) {
			logger.debug("开始生成组件ID=[{}]执行实例", comp_id);
			ModuleInfo module = XmlReader.readerModule(comp_id);
			template.setParams(module.getParams());
			if (!Assert.isEmpty(param_list)) {
				ParamInfo[] param_infos = param_list;
				env = tpCompSrv.getEnvByParam(param_infos);
				template.setParams(param_infos);
			}
			// ENV中加入数据源
			System.out.println("size: "+soc_list.size());
			for (UuSocInfo exsoc : soc_list) {
				DtSourceInfo dtsoc = dtSocService.getInfoByKey(exsoc.getExe_soc_name());
				checkModuleType(module, dtsoc);
				module.setDt_source(dtsoc);
				template.addSub_moduleInfo(module);
				env.addStageNode(i, dtsoc);
				i++;
			}
		}
		InstanceInfo inst = ProcessManager.instance.prepare(template, env);
		Process process = ProcessManager.instance.buildProcess(inst);
		...
		for (int j = 0; j < total; j++) {
			...
			try {
				...
				// 更新执行信息
				buildScriptExeDaoSrv.updateExeMsgByKey(msg, inst_id, j);
				if (result.getStatus() == CMD_STATUS.SUCCEED) {
					buildScriptExeDaoSrv.updateExeEndBkTmByKey(
							JaDateTime.now(), EXE_STATUS.EXECUTED,
							EXE_RESULT.SUCCESS, inst_id, j);
					cmsvc.getSession().commitAndResume();
					// 插入耗时
					updateExeCostTm(inst_id, j);
				} else {
					...
					buildScriptExeDaoSrv.updateExeEndBkTmByKey(
							JaDateTime.now(), EXE_STATUS.EXECUTED,
							EXE_RESULT.FAIL, inst_id, j);
					cmsvc.getSession().commitAndResume();
					// 插入耗时
					updateExeCostTm(inst_id, j);
					// 修改构建的时间和状态
					buildScriptDaoSrv.updateScriptStatusInfoByKey(
							JaDateTime.now(), EXE_STATUS.EXECUTED,
							EXE_RESULT.FAIL, work_id, script_bk_seq,
							script_type);
					break;
				}
			} catch (Exception e) {
				...
				buildScriptDaoSrv.updateScriptStatusInfoByKey(JaDateTime.now(),
						EXE_STATUS.EXECUTED, EXE_RESULT.FAIL, work_id,
						script_bk_seq, script_type);
				break;
			}
		}
		...
		if (mark) {
			buildScriptDaoSrv.updateScriptStatusInfoByKey(JaDateTime.now(),
					EXE_STATUS.EXECUTED, EXE_RESULT.SUCCESS, work_id,
					script_bk_seq, script_type);
			cmsvc.getSession().commitAndResume();
			BuildScriptInfo script_exe = buildScriptDaoSrv.getScriptByKey(
					work_id, script_bk_seq, script_type);
			JaDateTime start = script_exe.getStart_bk_tm();
			JaDateTime end = script_exe.getEnd_bk_tm();
			int time_used = getCostTm(start, end);
