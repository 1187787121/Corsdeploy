/**
 * Title: PageUncheckWorkAction.java
 * File Description:查询待复核任务 
 * @copyright: 2014
 * @company: CORSWORK
 * @author: tlw
 * @version: 1.0
...
/**
 * Class Description:查询待复核任务
 * 
 * @author tlw
 */
public class PageUncheckWorkAction extends
	...
	protected PageUnhandleWorkOutputBean doAction(
		...
		List<WkDealStateInfo> uncheck_work_list = wkPubSrv.pageUncheckWork(
				input.getOrg_user_id(), start_recd, limit_recd);
		output.setUnhandle_work_list(uncheck_work_list);
		output.setAll_recd(wkPubSrv.countUncheckWork(input.getOrg_user_id()));
		return output;
	}
	...
	protected String getLogTxt(PageUnhandleWorkInputBean input) {
		...
		return lgsrv.getLogTxt("查询待复核任务", lst_val);
	}
