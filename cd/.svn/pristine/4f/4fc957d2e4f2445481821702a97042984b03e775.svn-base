...
public class PythonSession
	...
	// 实现类型 主要是因为 这里要区分pyhon2 还是pyhon3
	...
	public PythonSession(ModuleSourceInfo module_source_info, int step_count,IMPL_TYPE imple_type,int execute_type, String remote_relative_dir) {
		super(step_count);
		this.module_source_info = module_source_info;
		Controller.getInstance().getInjector().inject(this);
		this.impl_type = imple_type;
		...
		try {
			...
			if (protocol_type == PROTOCOL_TYPE.SSH) {
				...
				sess = new AgentRSession(dt_info.getSoc_ip(), dt_info.getSoc_port(),
						impl_type, step_count, this.EXECUTE_TYPE, true, remote_relative_dir);
			} else {
	...
	public ShExecRsBean sendCmds(String[] cmds, String remote_relative_dir) {
		...
		if (protocol_type == PROTOCOL_TYPE.AGENT) {
			...
			CmdsExecuteUtil execute = new CmdsExecuteUtil(this.impl_type, ftp_info, remote_relative_dir);
			String[] remote_file = execute.uploadFile(cmds);
			sess.sendCmd("chmod 777 " + remote_file[0]);
			String suffix = impl_type == IMPL_TYPE.PYTHON2?"python2":"python3";
			ShExecRsBean result = sess.sendCmd(suffix+" "+remote_file[0]);
	...
	public void interactRun(String[] cmds, boolean is_read, String remote_relative_dir, boolean sensitive_flag)
		...
		if (protocol_type == PROTOCOL_TYPE.AGENT) {
			...
			if (is_read) {
				DtSourceInfo ftp_info = dtSocService.changeDtSource(dt_info);
				CmdsExecuteUtil execute = new CmdsExecuteUtil(impl_type, ftp_info, remote_relative_dir);
				String[] remote_file = execute.uploadFile(cmds);
				sess.sendCmd("chmod 777 " + remote_file[0]);
				String suffix = impl_type == IMPL_TYPE.PYTHON2?"python2":"python3";
				sess.asyncRunStage(new String[] { suffix+" "+remote_file[0] }, is_read, sensitive_flag);
			} else {
