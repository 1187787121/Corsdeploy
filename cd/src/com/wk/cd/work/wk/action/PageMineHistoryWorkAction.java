/**
 * Title: PageMineHistoryWorkAction.java
 * File Description: 分页查询我的历史任务列表
 * @copyright: 2016
 * @company: CORSWORK
 * @author: Xul
 * @version: 1.0
...
import com.wk.cd.work.wk.bean.PageMineHistoryWorkInputBean;
import com.wk.cd.work.wk.bean.PageMineHistoryWorkOutputBean;
...
/**
 * Class Description: 分页查询我的历史任务列表
 * @author Xul
 */
public class PageMineHistoryWorkAction extends ActionBasic<PageMineHistoryWorkInputBean, PageMineHistoryWorkOutputBean>{
	 ...
	 */
	@Override
	protected PageMineHistoryWorkOutputBean doAction(
			PageMineHistoryWorkInputBean input) {
		logger.info("*********PageMineHistoryWorkAction Begin**********");
		PageMineHistoryWorkOutputBean output = new PageMineHistoryWorkOutputBean();
		...
		Assert.assertNotEmpty(crt_user_id,
				PageMineHistoryWorkInputBean.ORG_USER_IDCN);
		Assert.assertNotEmpty(start_recd,
				PageMineHistoryWorkInputBean.START_RECDCN);
		Assert.assertNotEmpty(limit_recd,
				PageMineHistoryWorkInputBean.LIMIT_RECDCN);
		
		List<HistoryWorkBean> mine_history_list = wkPubSrv.pageMineHistoryWork(crt_user_id,start_recd,limit_recd);
		int all_recd = wkPubSrv.countMineHistoryWork(crt_user_id);
		if(!Assert.isEmpty(mine_history_list)){
			for (int i = 0; i < mine_history_list.size(); i++) {
				//查询任务处理明细
				List<WkDealDetailBean> detail_bean_list = wkDealDetailDaoService.queryDealDetailByWorkSeq(mine_history_list.get(i).getPend_work_seq());
				if(Assert.isEmpty(detail_bean_list)){
					throw new RecordNotFoundException().addScene("TABLE", "任务处理明细表").addScene("FIELD", "");
				}
				//查询任务审批流程表
				List<WkWorkProcessInfo> process_info_list = wkproDaos.getInfoListByKey(mine_history_list.get(i).getPend_work_seq());
				...
				for(int j = 0 ; j < process_info_list.size(); j++){
					...
					if(j == 0){
						bean.setDeal_bk_date(mine_history_list.get(i).getCrt_bk_date());
						bean.setDeal_bk_time(mine_history_list.get(i).getCrt_bk_time());
						bean.setDeal_user_cn_name(mine_history_list.get(i).getCrt_user_cn_name());
						bean.setDeal_bk_desc(mine_history_list.get(i).getApply_bk_expl());
				...
				mine_history_list.get(i).setWork_detail_list(work_detail_list);
			}
		}
		output.setMine_history_list(mine_history_list);
		output.setAll_recd(all_recd);
		
		logger.info("*********PageMineHistoryWorkAction End**********");
		return output;
	}
	 ...
	 */
	@Override
	protected String getLogTxt(PageMineHistoryWorkInputBean input) {
		List<String> lst_val = new ArrayList<String>();
		lst_val.add(input.getOrg_user_id());
		return lgsrv.getLogTxt("分页查询我的历史任务列表", lst_val);
	}
