/**
 * Title: PageMineHistoryUncheckAction.java
 * File Description: 分页查询我的历史复核任务列表
 * @copyright: 2016
 * @company: CORSWORK
 * @author: Xul
 * @version: 1.0
...
import com.wk.cd.work.wk.bean.PageMineHistoryUncheckInputBean;
import com.wk.cd.work.wk.bean.PageMineHistoryUncheckOutputBean;
...
/**
 * Class Description: 分页查询我的历史复核任务列表
 * @author Xul
 */
public class PageMineHistoryUncheckAction extends ActionBasic<PageMineHistoryUncheckInputBean, PageMineHistoryUncheckOutputBean>{
	...
	/** 
	 * Description: 分页查询我的历史复核任务列表
	 * @param input
	 * @return 
	 */
	@Override
	protected PageMineHistoryUncheckOutputBean doAction(
			PageMineHistoryUncheckInputBean input) {
		logger.info("**********PageMineHistoryUncheckAction Begin**********");
		PageMineHistoryUncheckOutputBean output = new PageMineHistoryUncheckOutputBean();
		...
		Assert.assertNotEmpty(crt_user_id,
				PageMineHistoryUncheckInputBean.ORG_USER_IDCN);
		Assert.assertNotEmpty(start_recd,
				PageMineHistoryUncheckInputBean.START_RECDCN);
		Assert.assertNotEmpty(limit_recd,
				PageMineHistoryUncheckInputBean.LIMIT_RECDCN);
		
		List<HistoryWorkBean> uncheck_history_list = wkPubSrv.pageMineUncheckOrUnauth(crt_user_id, DEAL_TYPE.RECHECK, start_recd, limit_recd);
		int all_recd = wkPubSrv.countMineUncheckOrUnauth(crt_user_id, DEAL_TYPE.RECHECK);
		if(!Assert.isEmpty(uncheck_history_list)){
			for (int i = 0; i < uncheck_history_list.size(); i++) {
				//查询任务处理明细
				List<WkDealDetailBean> detail_bean_list = wkDealDetailDaoService.queryDealDetailByWorkSeq(uncheck_history_list.get(i).getPend_work_seq());
				if(Assert.isEmpty(detail_bean_list)){
					throw new RecordNotFoundException().addScene("TABLE", "任务处理明细表").addScene("FIELD", "");
				}
				//查询任务审批流程表
				List<WkWorkProcessInfo> process_info_list = wkproDaos.getInfoListByKey(uncheck_history_list.get(i).getPend_work_seq());
				...
				for(int j = 0 ; j < process_info_list.size(); j++){
					...
					if(j == 0){
						bean.setDeal_bk_date(uncheck_history_list.get(i).getCrt_bk_date());
						bean.setDeal_bk_time(uncheck_history_list.get(i).getCrt_bk_time());
						bean.setDeal_user_cn_name(uncheck_history_list.get(i).getCrt_user_cn_name());
						bean.setDeal_bk_desc(uncheck_history_list.get(i).getApply_bk_expl());
				...
				uncheck_history_list.get(i).setWork_detail_list(work_detail_list);
			}
		}
		
		output.setUncheck_history_list(uncheck_history_list);
		output.setAll_recd(all_recd);
		
		logger.info("**********PageMineHistoryUncheckAction End**********");
		return output;
	}
	 ...
	 */
	@Override
	protected String getLogTxt(PageMineHistoryUncheckInputBean input) {
		List<String> lst_val = new ArrayList<String>();
		lst_val.add(input.getOrg_user_id());
		return lgsrv.getLogTxt("分页查询我的历史复核任务列表", lst_val);
	}
